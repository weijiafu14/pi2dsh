...
| [`pi-code`](https://www.npmjs.com/package/pi-code) | 仓库里的 Claude Code 配置原样在 DSH 生效：`settings.json` 的 `env` 与 `PreToolUse` hook、`CLAUDE.md` 的 `@import`（文本进入请求的系统提示，不是模型自己读文件）、`.claude/skills`、`.claude/rules`（常驻与 `paths:` 限定）、output styles、`.mcp.json` 服务器（pi-code 自有 MCP 客户端）、`.claude/commands` 的 `/name` 与 `slash_command` 工具两条路；首个会话在 DSH 原生问答卡里回答 "Trust this project?"。`.claude/agents` 的 `Task` 子代理不可用（pi-code 拉起的是 Pi CLI） | CLI + Web | [`claude-code-config`](examples/claude-code-config/) |
