...
export const HOST_IMPORT_RULES: Readonly<Record<string, Readonly<Record<string, Rule>>>> = Object.freeze({
  ...
  'pi-ai': Object.freeze({
    ...
    complete: rule('partial', 'Real model calls through the DSH llm bridge (the host installs it at mount); without a mounted llm service the call fails explicitly. Provider SDKs are never loaded. Pi package transports receive systemPrompt from either the legacy explicit slot or the leading DSH system message; unsupported in-history system updates fail explicitly.'),
    completeSimple: rule('partial', 'Legacy pi-ai/compat unified-options completion on the same DSH llm route; reasoning maps to native reasoningEffort, omitted optional thinking requests off where supported, and cancellation and token limits pass through. Missing host llm fails explicitly.'),
    streamSimple: rule('partial', 'Legacy pi-ai/compat unified-options streaming on the same DSH llm route, with native reasoningEffort, cancellation and token limits.'),
    stream: rule('partial', 'Real streaming model calls through the DSH llm bridge (the host installs it at mount); without a mounted llm service the call fails explicitly. Provider SDKs are never loaded. Pi package transports receive systemPrompt from either the legacy explicit slot or the leading DSH system message; unsupported in-history system updates fail explicitly.'),
...
export const CONTEXT_RULES: Readonly<Record<string, SurfaceRule>> = Object.freeze({
  ...
  hasPendingMessages: surface('full', 'Reads the public DSH nextStep and nextTurn inbox lists (legacy hasPending is a fallback) — next-step plus next-turn input — which is exactly Pi\'s steering plus follow-up queue.', 'Reads the DSH agent inbox — next-step plus next-turn queues — which is the same queue sendMessage/sendUserMessage write into.'),
...
export const API_RULES: Readonly<Record<string, SurfaceRule>> = Object.freeze({
  registerTool: {
    level: 'partial',
    detail: 'Registered as a native DSH tool. Text and image results use native DSH content/attachments; Web image cards use the session-authorized host loadImage callback when available, with the legacy session RPC retained for older hosts; unsupported JSON Schema constraints and Pi-only error details are explicitly degraded.',
  ...
  exec: {
    level: 'partial',
    detail: 'OS execution stays on ctx.subprocess with 64 MiB per output stream. On the local POSIX provider, a private per-execution pi -p facade translates print prompts, installed -e paths, model/thinking selection and tool restrictions into native DSH child agents; disconnect/timeout cancels the child. Unsupported CLI flags fail explicitly. --no-session suppresses Pi file export/indexing, but the native DSH child audit persists. Remote providers keep their own environment and execution path unchanged.',
    design: 'ctx.subprocess owns OS processes. A local print client carries arguments/stdout through an authenticated, private execution-scoped socket to the existing ctx.agents-based SDK bridge; no Pi model runtime or alternate provider transport is started. Native parentAgent ownership prevents duplicate extension mounting, and only installed extension paths can be selected.',
  },
  getActiveTools: {
    level: 'partial',
  ...
  getCommands: {
    level: 'partial',
    detail: 'Returns this package\'s registered commands plus Pi-shaped skill descriptors for file-backed, user-invocable skills visible in the current native DSH scope. It does not enumerate unrelated native slash commands.',
    design: 'Reads the package command map and a native skill metadata snapshot refreshed at session startup and command boundaries through ctx.skills.list/get. Skill descriptors preserve actual source paths; opaque non-file skill bodies are not assigned invented paths.',
  },
  setModel: {
    level: 'partial',
...
export const EVENT_RULES: Readonly<Record<string, SurfaceRule>> = Object.freeze({
  ...
  session_shutdown: { level: 'partial', detail: 'Async Cordis teardown joins pending message projection and ordered shutdown handlers; reload dispatches reason=reload before remount. Duplicate disposal notifications are suppressed. During full process shutdown other host services may already be unavailable, so model-dependent exit work is not guaranteed.', design: 'Uses the public async Cordis effect disposer while the native session still exists, with agent/disposed as the legacy fallback. The host does not expose a pre-shutdown gate that keeps every model and credential service alive until plugin callbacks complete.' },
  ...
  message_update: {
    level: 'partial',
    detail: 'Projected from live agent/assistant-stream frames on newer hosts and durable assistant/chunk events on older hosts, with retry-attempt isolation and canonical Pi text_delta events; reasoning is not mixed into text; Pi\'s full AgentMessage accumulation state is approximated.',
    design: 'New live frames and old durable chunks feed the same projection; a host using live frames is not counted twice. Text resets between attempts. Thinking and tool-argument streaming are not claimed by this text-only projection. Pi\'s full AgentMessage accumulation state is approximated, not reconstructed.',
  },
